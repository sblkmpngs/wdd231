...
async function getWeatherForecast() {
  try {
    ...
    const forecastDays = Array.from(daysMap.values()).slice(0, 3);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('data/members.json')
    ...
    .then(data => {
      ...
      const filteredBusinesses = data.businesses.filter(biz =>
        biz.membership.toLowerCase() === 'gold' || biz.membership.toLowerCase() === 'silver'
      );

      const selected = filteredBusinesses
      ...
      selected.forEach(biz => {
        ...
        let membershipClass = '';
        switch (biz.membership.toLowerCase()) {
          case 'gold':
